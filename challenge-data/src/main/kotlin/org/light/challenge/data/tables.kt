...
object Rule : Table() {
    val id = integer("id").autoIncrement().primaryKey()
    val min_amount = decimal("min_amount", precision = 10, scale = 2)
    val max_amount = decimal("max_amount", precision = 10, scale = 2)
    val is_manager_approval_required = bool("is_manager_approval_required")
    val related_department_id = (integer("related_department_id") references Department.id)
    val destination_department_id = (integer("destination_department_id") references Department.id)
    val workflow_id = (integer("workflow_id") references Workflow.id)
}
